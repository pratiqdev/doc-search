...
({ className, type, fullWidth = false, searchable, ...props }, ref) => {
    ...
    return (
      <div
        ...
        <div className="flex flex-col overflow-hidden text-xs">
          ...
          <p className="font-light tracking-wide truncate whitespace-nowrap h-4" dangerouslySetInnerHTML={{ __html:highlightWord((context?.length && context.length > 5 ? context : subtitle) ?? '', searchValue)  }} />
        </div>
      </div>
    )
  }
  ...
  // &                                                                                                                                                                                                      
  React.useEffect(() => {
    setSearchResults(filterResults(searchValue))
    ...
    setSuggestions((searchValue && searchValue.length) 
      ? termSuggestions.filter((term:string, index:number) => {
          console.log('suggest | searchValue includes space - suggesting on last word:', searchValue)
          
          return term.startsWith(searchValue.trim().includes(' ') ? searchValue.trim().split(' ')[1] : searchValue) 
      ...
      <DialogContent className="sm:max-w-[425px] max-w-[600px] bg-slate-50 rounded-lg md:rounded-lg  flex flex-col items-center gap-1 p-4">
        ...
        {/* //! Link to a page about how this search was built */}
          <div className="text-xs flex-1 text-right pb-1 text-slate-600 cursor-pointer hover:underline">
              Inspired by <b className="font-semibold text-indigo-600">Algolia</b> and  <b className="font-semibold text-indigo-600">EveryDocs</b>
            </div>
        </div>
